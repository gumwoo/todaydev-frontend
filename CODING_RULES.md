...
- API 연동 전 반드시 `API_CONTRACT.md`를 먼저 확인한다.
- TypeScript 타입, API 함수, query key, SSE event name은 `API_CONTRACT.md`를 기준으로 만든다.
- 백엔드 응답을 화면에서 임의로 추측하지 않는다.
- 계약과 실제 응답이 다르면 임시 우회 코드를 만들지 말고 계약 또는 백엔드 구현을 먼저 맞춘다.
- 프론트에서 계약 변경이 필요하면 백엔드 `API_CONTRACT.md`도 같은 내용으로 갱신한다.
